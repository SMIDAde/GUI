...
# select "make commands" which will be shown, if only "make" is written
define PRINT_HELP_PYSCRIPT
import re, sys

for line in sys.stdin:
		# scan a line for a "some-word: ## a longer description"
...
dataset-bioeye:  ## Initialize bioeye dataset
	git submodule update --init --remote -- data/BioEye2015_DevSets
	cd data/BioEye2015_DevSets && git lfs pull
dataset-rigas:  ## Initialize rigas dataset
	git submodule update --init --remote -- data/RigasEM
	cd data/RigasEM && git lfs pull
dataset-dyslexia:  ## Initialize dyslexia dataset
	git submodule update --init --remote -- data/Dyslexia
	cd data/Dyslexia && git lfs pull
dataset-whl:  ## Initialize whl dataset
	git submodule update --init --remote -- data/where_humans_look
	cd data/where_humans_look && git lfs pull
dataset-biometricsDS:  ## Initialize biometricsDS dataset
	git submodule update --init --remote -- data/data_cleaned_biometrics_ds
	cd data/data_cleaned_biometrics_ds && git lfs pull
