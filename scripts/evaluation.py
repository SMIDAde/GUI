...
from schau_mir_in_die_augen.general.io_functions import load_pickle_file
...
def main(args):
    ...
    # if args.plot_RDF_tree > 0:
    #     if args.train_top_features > 0:
    #         features = 'top'
    #     else:
    #         features = 'all'
    #     eva.plot_decision_tree(features, args.plot_RDF_tree, get_user_ids(labeled_feature_values_train))
...
def eval_subset(labeled_feature_values, eva, args, subset_length):
    ...
    pickel_file_path = res['config']['modelfile']
    print("pickel_file_path", pickel_file_path)
    try:
        res['config']['modelfile'] = pickel_file_path.split('scripts/')[1]
    except:
        print("pickle file path does not contain scripts substring")
    result_name = '&'.join([f'{k}={v}' for k, v in res['config'].items()])
    res['params'] = result_name

    train_accuracy_res = eva.train_accuracy
    res['Train_ccuracy_balanced_fix'] = train_accuracy_res['Accuracy_balanced_fix']
    res['Train_accuracy_balanced_sac'] = train_accuracy_res['Accuracy_balanced_sac']
    res['Train_accuracy_balanced_avg_fix_sac'] = (res['Train_ccuracy_balanced_fix']+res['Train_accuracy_balanced_sac'])/2
    res['Train_accuracy_balanced'] = train_accuracy_res['Accuracy_balanced']

    if args.save_eval_res_path is not None:
        filename = 'results/' + args.save_eval_res_path + '/file_' + str(datetime.datetime.now()).replace(':', '') + '.json'
