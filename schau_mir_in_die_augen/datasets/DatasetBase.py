...
class DatasetBase(ABC):
    ...
    def load_trajectory(self, user: str, case: str) -> Trajectory:
        ...
        if self.data_kind != 'pixel_shifted':
            if self.data_kind == 'angle_deg':
                ...
                raise Exception('Please create case to handle if this fails.')
        ...
        try:
            trajetory = self.modify_trajectory(Trajectory(
                ...
                label_by_dataset=self.get_label(user),
    ...
    def load_trajectories(self, users: list, cases: list):
        """ load multiple data sets"""
        # print("inside load_trajectories of datasetbase")
        # print("No. of healthy males in trajectories_train: ",
        #       len([u for u in users if u.endswith('3')]))
        # print("No. of healthy females in trajectories_train: ",
        #       len([u for u in users if u.endswith('4')]))
        # print("No. of dyslexic males in trajectories_train: ",
        #       len([u for u in users if u.endswith('1')]))
        # print("No. of dyslexic females in trajectories_train: ",
        #       len([u for u in users if u.endswith('2')]))
