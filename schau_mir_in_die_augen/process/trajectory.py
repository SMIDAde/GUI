...
class TrajectoryBase:
    ...
    def __init__(self, xy):
        ...
        # copy() makes sure we have independent data, which can not been accessed from the outside.
...
class TrajectoryFOV(TrajectoryModify):
    ...
    def _set_kind(self, kind):
        # overwrites set_kind without change
        # todo: Maybe this method should be used, or overwritten. convert_to can do the same.
...
class TrajectoryConvert(TrajectoryFOV):
    ...
    def convert_to(self, kind):
        """ Change the stored type of data to new type """
    ...
    def get_trajectory(self, kind, inplace: bool = False, data_slice: slice = None, return_copy: bool = True):
        ...
        if kind == 'pixel':
            ...
            """ move the trajectory out of the center in quadrant 1
                if pixel_image is selected: invert y component before.
            """
            if inplace:
...
class TrajectoryMore:
    ...
    def __init__(self, user: str = None, case: str = None, gender: str = None, label_by_dataset: str = None,
                 age: str = None):
        """ Optional Parameters:
