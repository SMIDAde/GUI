...
todo: Was it a decision on purpose to have the feature_names default argument mutable?
    If not, change it.
"""
...
def angle_among_3consecutive_Points(xy):
    ...
    todo: Rename this method: Points => points
...
def general_gaze_points_features(xy, sample_rate, screen_params):
    ...
    return \
        ...
        + [dispersion] \
        + num_change_direction, general_gaze_points_features_header
...
def direction_changes_names(prefix=""):
    return ["{}_num_change_direction".format(prefix)] \
           + ["{}_num_change_direction_threshold_{}".format(prefix, i) for i in range(1, 21)]
...
def non_distributional_features(fix_vec, sampleRate, fix_dis_vec, rec_duration, fix_angular_vel_vec, fix_sac):
    ...
    return \
        ...
        + angular_deacceleration_feats \
        + acceleration_feats \
        + deacceleration_feats, non_distributional_fixation_features_headers
